package com.example.gymprogress.ui.theme

import androidx.compose.ui.graphics.Color

val PrimaryGreen = Color(0xFF5E8E3E)
val SecondaryGreen = Color(0xFF8BC34A)
val DarkGreen = Color(0xFF33691E)
val HighlightGreen = Color(0xFFAED581)
val SurfaceSoft = Color(0xFFF6F8F4)
val TextPrimary = Color(0xFF1F2937)
val TextSecondary = Color(0xFF4B5563)
val OutlineLight = Color(0xFFE0E4E8)
val ExerciseCompletedLight = Color(0xFFCFE8D6)
val ExerciseCompletedDark = Color(0xFF1F3B2A)
val ActiveGlowBlue = Color(0xFF47E6FF)
val RecommendedExerciseYellow = Color(0xFFFFD54F)
